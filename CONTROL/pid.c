...
void PID_Postion_Cal(PID_TYPE*PID,float target,float measure)
{
	...
	if(RC_Control.BUTTON>1400&&RC_Control.THROTTLE >= 180)  //飞机解锁之后再加入积分,防止积分过调
	{
...
{
	//ROLL轴
	PID_ROL_Angle.P = 7.19;			//4.5				6.5 较好		2022/8/19 23:00 可以飞行较长一段时间
	PID_ROL_Angle.I = 0.000219;			//0   0.000719		0.000219
	PID_ROL_Angle.D = 9.1;				//  10 大
	PID_ROL_Rate.P = 0.9;
  PID_ROL_Rate.I = 0;
	PID_ROL_Rate.D = 1.7;
	...
	PID_ROL_Angle.Ilimit = 20;    //Roll轴角度积分范围			35
	PID_ROL_Angle.Irang = 15;    //Roll轴角度积分限幅度（由于电机输出有限，所以积分输出也是有限的）		200

	//PITCH轴
	PID_PIT_Angle.P = 7.19;
	PID_PIT_Angle.I = 0.000219;
	PID_PIT_Angle.D = 9.1;
  PID_PIT_Rate.P = 0.9;
	PID_PIT_Rate.I =0;
	PID_PIT_Rate.D = 1.7;
	...
	PID_PIT_Angle.Ilimit = 20;    //Roll轴角度积分范围
	PID_PIT_Angle.Irang = 15;    //Roll轴角度积分限幅度（由于电机输出有限，所以积分输出也是有限的）
